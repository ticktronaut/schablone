...
class smd_container(generic.generic):
	...
	def __init__(self):
		...
		self.cpt_tspan = {'title' : '', 'value' : '', 'package' : '', 'tolerance' : '', 'temperature_coefficient' : '', 'power' : ''} 
#		self.cpt_flowpara = {}
	...
	def save(self, fn = None):
		...
		fn_qr = fn + '_qr' + '.png'
		os.system('uuidgen | tr [[:upper:]] [[:lower:]] | tr -d \'-\'  | sed -e \'s/\(.\{12\}\).*/\\1/\' | dmtxwrite -s 16x16 -o ' + fn_qr)
		...
		self.layer.add( pkg_resources.resource_filename('schablone','templates/label/smd_container/' + self.label_type + '/font.svg') )

		if not self.cut:
			self.layer.add( pkg_resources.resource_filename('schablone','templates/label/smd_container/'+ self.label_type + '/frame.svg') )
		else:
#			self.layer.default_lr = 'cut'
			self._fn_cut = super(smd_container, self)._fn_sub_str(self._fn, "_cut")
			self.layer.add( pkg_resources.resource_filename('schablone', 'templates/label/smd_container/'+ self.label_type + '/frame_cut.svg'), 0.0, 0.0, 1.0, 'cut')
...
class box(generic.generic):
	def __init__(self):
		super(box, self).__init__()

		self.content = box_content_container()

		self.fn_template_default = pkg_resources.resource_filename('schablone', 'templates/label/box/template_default.svg')
		self.fn_template_extended = pkg_resources.resource_filename('schablone', 'templates/label/box/template_extended.svg')
	...
	def save(self, fn = None):
		...
		self.layer.add( pkg_resources.resource_filename('schablone','templates/label/box/template_default.svg') )
		...
		if self.label_type == 'extended':
			...
			self.layer.add( pkg_resources.resource_filename('schablone','templates/label/box/template_extended.svg') )
